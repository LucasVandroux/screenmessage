function FP_Position = findPositionFinderPattern4(frame, step, error, unit_min)
%Analyse an image to find the position of the center of the 4 Finder Pattern of the QRcode.
...
%      FP_Position_test = FP_Position
    ...
    if size(FP_Position, 1) >= 4
         % Sort the matrix to ouput the 3 bests
        FP_Position = sortrows(FP_Position,-4);
        FP_Position = FP_Position(1:4,1:4);

        % Sort the 4 best point such that the first is the left-top, the second
        % right-top, the third is the left-bottom and the last one is
        % the right-bottom
        FP_Position = sortrows(FP_Position, 2);
        FP_Position(1:2,1:3) = sortrows(FP_Position(1:2,1:3), 1);
        FP_Position(3:4,1:3) = sortrows(FP_Position(3:4,1:3), 1);
    else
         FP_Position = [];
    end
